...
const answers: Record<string, AssistantAnswer> = {
  ...
  "Could United swap the aircraft?": {
    answer: [
      ...
      "TailWind puts the chance at about 24%. Airlines usually wait until a delay is clearer before swapping. If the storms pause departures, that odds could rise.",
    ],
  ...
  "Should I leave for the airport later?": {
    answer: [
      ...
      "If United confirms a 40-minute delay, you could leave around 5:15 PM. TailWind will tell you the moment that changes.",
    ],
  ...
  "What happens if this gets cancelled?": {
    answer: [
      ...
      "TailWind would alert you before the airline's own notice in most cases, so you could rebook first.",
    ],
  ...
  "What happens if I miss my connection?": {
    answer: [
      ...
      "TailWind puts the chance of missing it at 41% today.",
    ],
...
export function answerFor(q: string): AssistantAnswer {
  ...
  if (l.includes("why") || l.includes("delay")) return answers["Why does TailWind think my flight will be delayed?"];
