...
/**
 * A real airliner silhouette (swept wings, fuselage, tail) drawn pointing
 * NORTH — so a map marker can be rotated by a compass heading directly, with
 * no counter-rotation. (RoutePlaneIcon above is the small route dart for
 * list rows; this is the bigger, recognisable one for the tracker map.) The
 * path is Material Design's "flight" icon (Apache 2.0). A dark outline is
 * drawn under the fill so it stays legible over land, ocean and clouds.
 */
const AIRPLANE = "M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z";

export function AirplaneGlyph({ size = 34, color, outline }: { size?: number; color: string; outline: string }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24">
      <Path d={AIRPLANE} fill={outline} stroke={outline} strokeWidth={2.6} strokeLinejoin="round" />
      <Path d={AIRPLANE} fill={color} />
    </Svg>
  );
}
